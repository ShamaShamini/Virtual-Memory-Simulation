...
NUM_PAGES = 8
NUM_FRAMES = 4
MAX_TRANSLATIONS = 10   # <- Limit to 10 inputs
# -------------------------------------------------
class VirtualMemoryGUI:
    def __init__(self, root):
        ...
        self.next_frame = 0
        self.translation_count = 0   # <-- Track number of processed addresses
    ...
    def setup_ui(self):
        ...
        # Footer
        footer = f"Page Size: {PAGE_SIZE} bytes | Max Address: {NUM_PAGES * PAGE_SIZE - 1} | Max 10 Inputs"
        tk.Label(self.root, text=footer, font=("Arial", 9), bg="#f0f0f0", fg="#7f8c8d").pack(pady=10)

    def translate(self):
        if self.translation_count >= MAX_TRANSLATIONS:
            messagebox.showwarning("Limit Reached", f"You have already processed {MAX_TRANSLATIONS} addresses.\nClick 'Reset' to start over.")
            return
        ...
        try:
            ...
            messagebox.showerror("Invalid", f"Address must be an integer between 0 and {NUM_PAGES * PAGE_SIZE - 1}")
            return
        ...
        # Success: count this translation
